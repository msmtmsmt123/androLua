...
local function checkPath(path)
  if path:find("^[^/][%w%./_%-]+$") then
    if not path:find("%.lua$") then
      path=string.format("%s/%s.lua",luacontext.luaDir,path)
    else
      path=string.format("%s/%s",luacontext.luaDir,path)
    end
  end
  return path
end
...
function thread(src,...)
  ...
  local luaThread
  if ... then
    luaThread=LuaThread(luacontext,src,true,Object{...})
  else
    luaThread=LuaThread(luacontext,src,true)
  end
  luaThread.start()
...
function task(src,...)
  ...
  args[select("#",...)]=nil
  local luaAsyncTask=LuaAsyncTask(luacontext,src,callback)
  luaAsyncTask.execute(args)
end

function timer(f,d,p,...)
  local luaTimer=LuaTimer(luacontext,f,Object{...})
  if p==0 then
    luaTimer.start(d)
  else
    luaTimer.start(d,p)
