		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				if (x != 0 && x != w - 1 && y != 0 && y != h - 1) {
					// G
					g1 = pix[x - 1 + (y - 1) * w] + 2 * pix[x + (y - 1) * w]
							+ pix[x + 1 + (y - 1) * w]
							- pix[x - 1 + (y + 1) * w] - 2
							* pix[x + (y + 1) * w] - pix[x + 1 + (y + 1) * w];
					g2 = pix[x - 1 + (y - 1) * w] + 2 * pix[x - 1 + (y) * w]
							+ pix[x - 1 + (y + 1) * w]
							- pix[x + 1 + (y - 1) * w] - 2
				 ...
				 * else { if (x == w - 1) { newpix[x + y * w] = pix[x - 1 + y *
				 * w]; } if (y == h - 1) { newpix[x + y * w] = pix[x + (y - 1) *
				 * w]; }0 }
				 */
			}
		}
