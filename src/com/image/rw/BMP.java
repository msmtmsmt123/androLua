	...
	public Bitmap readImage() {
		...
		try {
			...
			if (compressionType == BI_RGB) {
				/*
				 * if (bitCount == 24) readRGB24(in, width, height, pixels);
				 * else { //readRGB(width, height, colorTable, bitCount, pixels,
				 * in); //img = ImageIO.read(new File(file)); }
				 */
			} else if (compressionType == BI_RLE8) {
				readRLE(width, height, colorTable, biBitCount, pixels, in,
						imageSize, 8);
