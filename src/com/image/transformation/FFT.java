...
public class FFT extends Transformation {
	...
	public int[] processing(int[] pix, int w, int h) {
		...
		for (int i = 0; i < h; i++) {
			for (int j = 0; j < h; j++) {
				a = matrix[i][j].getA();
				b = matrix[i][j].getB();
				newpix[i * w + j] = (int) (Math.sqrt(a * a + b * b) / 100);
				// System.out.print(a + "\t");
			}
		}
	...
	public Complex[][] fft(Complex matrix[][], int w, int h) {
		...
		int r = (int) (Math.log10(w) / Math.log10(2));
		// 进行行傅里叶变换
		for (int i = 0; i < h; i++) {
			...
			if (l != r) {
				...
				for (int i = 0; i < Math.pow(2, l - 1); i++) {
					for (int k = 0; k < d; k++) {
						...
						temp[l][x1] = Complex.add(temp[l - 1][x1],
								Complex.multiply(w, temp[l - 1][x2]));

						w.setA(-Math.cos(-2 * Math.PI * p / n));
						w.setB(-Math.sin(-2 * Math.PI * p / n));
						temp[l][x2] = Complex.add(temp[l - 1][x1],
								Complex.multiply(w, temp[l - 1][x2]));
					}
				}
			} else {
				for (int k = 0; k < n / 2; k++) {
					...
					w.setA(Math.cos(-2 * Math.PI * p / n));
					w.setB(Math.sin(-2 * Math.PI * p / n));
					temp[l][p] = Complex.add(temp[l - 1][x1],
							Complex.multiply(w, temp[l - 1][x2]));
	...
	/*
	 * public int[] toPix(Complex[] fftData, int iw, int ih) { int[] pix = new
	 * int[iw * ih];
	 * 
	 * int u, v, r; for (int j = 0; j < ih; j++) { for (int i = 0; i < iw; i++)
	 * { double tem = fftData[i + j * iw].getA() * fftData[i + j * iw].getA() +
	 * fftData[i + j * iw].getB() * fftData[i + j * iw].getB(); r = (int)
	 * (Math.sqrt(tem) / 100); if (r > 255) r = 255;
	 * 
	 * if (i < iw / 2) u = i + iw / 2; else u = i - iw / 2; if (j < ih / 2) v =
	 * j + ih / 2; else v = j - ih / 2;
	 * 
	 * pix[u + v * iw] = r;//255 << 24 | r << 16 | r << 8 | r; } } return pix; }
	 */
	/**
