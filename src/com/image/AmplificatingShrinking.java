...
public class AmplificatingShrinking {
	 ...
	 * @param k2
	 */
	public static void bilinearityInterpolation(String srcPath,
			String distPath, CompressFormat formatName, float k1, float k2) {
	...
	public static Bitmap bilinearityInterpolation(Bitmap img, float k1, float k2) {
			...
			for (int i = 0; i < w - 1; i++) {
				...
				for (int l = 0; l < d2; l++) {
					for (int k = 0; k < d1; k++) {
						if (0 == l) {
							if (j < h - 1 && newpix[y0 * m + x0 + k] == 0) {
								c = (float) k / d1;
								r = Color.red(newpix[y0 * m + x0])
										+ (int) (c * (Color.red(newpix[y0 * m
												+ x1]) - Color.red(newpix[y0
												* m + x0])));// newpix[(y0+l)*m
																// + k]
								g = Color.green(newpix[y0 * m + x0])
										+ (int) (c * (Color.green(newpix[y0 * m
												+ x1]) - Color.green(newpix[y0
												* m + x0])));
								b = Color.blue(newpix[y0 * m + x0])
										+ (int) (c * (Color.blue(newpix[y0 * m
												+ x1]) - Color.blue(newpix[y0
												* m + x0])));
								newpix[y0 * m + x0 + k] = Color.rgb(r, g, b);
							}
							if (j + 1 < h && newpix[y1 * m + x0 + k] == 0) {
								c = (float) k / d1;
								r = Color.red(newpix[y1 * m + x0])
										+ (int) (c * (Color.red(newpix[y1 * m
												+ x1]) - Color.red(newpix[y1
												* m + x0])));
								g = Color.green(newpix[y1 * m + x0])
										+ (int) (c * (Color.green(newpix[y1 * m
												+ x1]) - Color.green(newpix[y1
												* m + x0])));
								b = Color.blue(newpix[y1 * m + x0])
										+ (int) (c * (Color.blue(newpix[y1 * m
												+ x1]) - Color.blue(newpix[y1
												* m + x0])));
							...
							c = (float) l / d2;
							r = Color.red(newpix[y0 * m + x0 + k])
									+ (int) (c * (Color.red(newpix[y1 * m + x0
											+ k]) - Color.red(newpix[y0 * m
											+ x0 + k])));
							g = Color.green(newpix[y0 * m + x0 + k])
									+ (int) (c * (Color.green(newpix[y1 * m
											+ x0 + k]) - Color.green(newpix[y0
											* m + x0 + k])));
							b = Color.blue(newpix[y0 * m + x0 + k])
									+ (int) (c * (Color.blue(newpix[y1 * m + x0
											+ k]) - Color.blue(newpix[y0 * m
											+ x0 + k])));
					...
					if (i == w - 2 || l == d2 - 1) { // 最后一列的计算
						c = (float) l / d2;
						r = Color.red(newpix[y0 * m + x1])
								+ (int) (c * (Color.red(newpix[y1 * m + x1]) - Color
										.red(newpix[y0 * m + x1])));
						g = Color.green(newpix[y0 * m + x1])
								+ (int) (c * (Color.green(newpix[y1 * m + x1]) - Color
										.green(newpix[y0 * m + x1])));
						b = Color.blue(newpix[y0 * m + x1])
								+ (int) (c * (Color.blue(newpix[y1 * m + x1]) - Color
										.blue(newpix[y0 * m + x1])));
		...
		Bitmap imgOut = Bitmap.createBitmap(m, n, config);
	...
	public static void biCubicInterpolationScale(String srcPath,
			String destPath, CompressFormat formatName, float k1, float k2) {
		Bitmap img = ImageDigital.readImg(srcPath);
		int w = img.getWidth();
		int h = img.getHeight();
		...
		int[] newpix = BiCubicInterpolationScale.imgScale(pix, w, h, destW,
				destH);
		Bitmap imgOut = Bitmap.createBitmap(destW, destH, config);
		imgOut.setPixels(newpix, 0, destW, 0, 0, destW, destH);
	...
	public static void filexIsometry(String srcPath, String distPath,
			CompressFormat formatName, int m, int n) {
		Bitmap img = ImageDigital.readImg(srcPath);
	...
	public static int[] shrink(int[] pix, int w, int h, int m, int n) {
		...
		for (int j = 0; j < n; j++) {
			for (int i = 0; i < m; i++) {
				...
				for (int k = 0; k < jj; k++) {
					for (int l = 0; l < ii; l++) {
						r = r + Color.red(pix[(jj * j + k) * w + (ii * i + l)]);
						g = g
								+ Color.green(pix[(jj * j + k) * w
										+ (ii * i + l)]);
						b = b
								+ Color.blue(pix[(jj * j + k) * w
										+ (ii * i + l)]);
					}
				}
				r = r / dd;
